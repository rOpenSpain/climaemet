...
[this article](https://rspatial.org/terra/analysis/4-interpolation.html) on how
...
# Interpolate temp
init_p <- test_day %>% vect() %>% as_tibble(geom="XY")

gs <- gstat(formula = tmed ~ 1, locations = ~x+y, data = init_p)

interp_temp <- interpolate(grd, gs, idp = 2.0)
...
ggplot() +
  ...
  labs(title="(interpolated) temperature",
       subtitle = "2021-01-08") 
```
...
temp_values <- interp_temp %>% pull(var1.pred) %>%
  range(na.rm=TRUE)
...
interp_list <-  lapply(dates, function(x){
  ...
  names(interp_day) <- paste0("interp_", format(thisdate))
...
# Facet map with January data only
jan_only <- which(grepl("2021-01", names(interp_rast)) == TRUE)

ggplot() +
  geom_spatraster(data = interp_rast %>% select(all_of(jan_only))) +
  facet_wrap(~lyr) +
  scale_fill_whitebox_c(palette = "pi_y_g", alpha=1) +
  theme_minimal() +
  labs(title = "Temperatures on Jan 2021")
