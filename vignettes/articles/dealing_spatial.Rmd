...
applications, so there is also a wealth of spatial data types. @cressie93
...
See @montero_spatial, for more details. In this work, we focus on geostatistical
data.
...
We download the data with **climaemet (\>= 1.0.0)** package [@climaemet] in R.
...
create a grid of locations and perform an interpolation.
...
following @montero_spatial we formulate this question: **How do we express in a
...
According to @montero_spatial, **kriging** aims to predict the value of a random
...
spatial interpolation. We recommend [this
