...
```{r choro, fig.cap="Max. temperature 12 Aug. 2021"}
...
ggplot() +
  geom_tile(data = kriged_df, 
            aes(x = coords.x1, 
                ...
                fill = var1.var)) +
  geom_sf(data = CCAA_utm, col = "black", fill = NA) +
  geom_sf(data = clim_data_clean_nodup, col = "black", shape = 1) +
  scale_fill_gradientn(
    # Special palette
    colours = c("green4", "white", hcl.colors(10, "PuRd", alpha = .7, rev = TRUE)),
    breaks = c(-Inf, 6, 10, 15, 20, 30, Inf),
    guide = guide_legend(reverse = TRUE, title = "Variance")
  ) +
  theme_light() +
  theme(
    panel.background = element_blank(),
...
GEMA, DE INCLUIR IDW CREO QUE PODRIA SER AQUI Y COMPARAR AMBAS INTERPOLACIONES

GEMA, PUEDES REVISAR LAS REFERENCIAS?
