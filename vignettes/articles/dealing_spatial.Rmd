...
are going to model the level of air temperature in Spain on **8 January 2021**.
...
Now we can start extracting the data. We select here the 
daily values of **8 January 2021**:
...
```{r selecttemp, fig.cap="AEMET Stations (excl. Canary Islands)"}
...
```{r choro, fig.cap="Min. temperature 8 Jan. 2021"}
...
summary(clim_data_clean$tmin) %>%
  as.matrix() %>%
  knitr::kable(
    digits = 3,
    caption = "Summary of min. temp (tmin) variable"
  )
```
...
```{r bubbleplot, fig.cap="Min. temperature - Quartiles" }
...
```{r hist, fig.cap="Min. temperature - Histogram"}
...
vgm <- variogram(tmin ~ 1, clim_data_clean_nodup_sp)
plot(vgm)
