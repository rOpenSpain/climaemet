...
However, for spatial analysis and visualization it can be useful to extend the point data
to cover the whole extent of Spain. In this article we explain a method to
interpolate climatic data using [Spatial
...
We use daily climatic data for winter 2020–2021 in Spain. Note that
in the first half of January, [Storm
Filomena](https://en.wikipedia.org/wiki/Storm_Filomena) brought unusually heavy
...
- Longitude/latitude coordinates are unprojected. When we project an object
  (for example, to a Mercator projection) we change the x/y values of every
  point — a projection transforms coordinates.
