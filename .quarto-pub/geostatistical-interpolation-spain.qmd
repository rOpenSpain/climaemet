...
author:
  - id: faviles
    ...
    degrees:
      - PhD
    note: "Full Professor of Statistics at the University of Castilla-La Mancha"
    roles:
      - investigation
    affiliation:
...
citation:
  ...
  pdf-url: https://dieghernan-geostatistical-interpolation-spain.share.connect.posit.cloud/geostatistical-interpolation-spain.pdf
copyright:
  holder: Gema Fernández-Avilés and Diego Hernangómez
  year: 2026
license: "CC BY"
...
The analysis loads the following R packages at the start so that later code can
focus on the geostatistical workflow:
...
The main data source is the **climaemet** package [@10261_250390]. The spatial
workflow uses the **mapSpain** [@R-mapspain], **sf** [@R-sf], **terra**
[@R-terra], **gstat** [@pebesma2004] and **geoR** [@R-geoR] packages. Supporting
data manipulation, classification and plotting use the **tidyverse**
[@R-tidyverse], **classInt** [@R-classInt] and **tidyterra** [@R-tidyterra]
packages. Later sections mention packages again only when they are central to
the method being introduced. Helper namespaces such as **knitr** [@R-knitr],
**dplyr** [@R-dplyr] and **scales** [@R-scales] appear in code where they avoid
ambiguity, but they are not discussed as methodological dependencies.
...
Daily climatological observations are retrieved with the **climaemet** package
(\>= 1.0.0). The package provides access to the Spanish State Meteorological
...
We retain the response variable and use the **mapSpain** package to represent
...
model. We use the **terra** package to represent the grid as a `SpatRaster`.
@hijmans2023 describes the corresponding interpolation workflow with the
...
temperature at grid-cell centers without observations.
...
the specified mean and covariance model, ordinary kriging provides the best
...
Several R packages support geostatistical analysis. This article uses the
**geoR** and **gstat** packages, including the extensions described by
@graler2016.
...
The resulting prediction surface, visualized with the **tidyterra** package,
...
evaluated empirically for the data and the prediction objective at hand. The
cross-validation results characterize performance at sampled locations, but they
do not establish accuracy at all unsampled grid cells. This analysis also
