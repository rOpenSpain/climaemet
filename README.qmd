...
AEMET OpenData is a REST API developed by AEMET that allows dissemination and
reuse of the Agency's meteorological and climatological information. For more
details, visit <https://opendata.aemet.es/centrodedescargas/inicio>.
...
Now the `apikey` argument in the functions has been deprecated. You may need to
...
format](https://tibble.tidyverse.org/). Also, the functions try to infer the
correct format of fields (for example, date/hour fields are parsed as date/time
objects and numeric fields are parsed as doubles).
...
Additionally, we can plot wind speed and direction over time for weather station
data.
