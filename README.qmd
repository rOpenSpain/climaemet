...
The `apikey` argument in the functions is now deprecated. You may need to
set your API key globally using `aemet_api_key()`. Note that you also need to
remove the `apikey` argument from old code.

### Now **climaemet** is tidy...

From `v1.0.0` onward, **climaemet** provides its results in
[**tibble** format](https://tibble.tidyverse.org/). The functions also try to
infer the correct format of fields. For example, date and hour fields are
