...
- Improvements in
  ...
  - New parameter `stack_reverse` for changing the order of the stacks
    on each petal.
- Minimal **ggplot2** version required is now `>= 3.5.0` as a
...
- `apikey` parameter has been deprecated on all the functions. Now the
  API Key is globally managed via an environment variable: see
...
- Results are parsed to the correct formats (numbers and dates when
  possible).
- Spatial support: New option `return_sf` would return `sf` objects
  instead of `tibble` objects. **sf**`(>= 0.9)` required, listed on
...
- [`aemet_last_obs()`](https://ropenspain.github.io/climaemet/reference/aemet_last_obs.md)
  now is vectorized and it can also retrieve all the stations at a
...
- New function
  ...
  (experimental). Old behavior can be reproduced with options
  `ggplot2 = FALSE`.
- Plot functions gains new parameters (`verbose` and `...`). Now it is
  possible to pass colors to the plotting functions.
- New example data sets: `climaemet_9434_climatogram`,
  `climaemet_9434_temp`, `climaemet_9434_wind`.
...
- Now palettes are generated with
  [`hcl.colors()`](https://rdrr.io/r/grDevices/palettes.html) (base
  **R**)
